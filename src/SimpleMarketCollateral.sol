...
/// @title SimpleMarketCollateral
/// @notice A simple collateral contract for Wildcat markets.
/// @dev This contract is a simplified version of the WildcatMarketCollateral contract.
///      It only supports deposits and post-termination collateral reclaims by the borrower.
contract SimpleMarketCollateral is ReentrancyGuard {
    ...
    event CollateralDeposited(uint256 amountDeposited);
    event CollateralReclaimed(uint256 amountReclaimed);
    event Liquidation(
        address liquidator,
    ...
    function reclaimCollateral() public onlyBorrower marketClosed {
        uint256 reclaimAmount = collateralAsset.balanceOf(address(this));
        if (reclaimAmount == 0) revert ZeroTokenBalance();

        collateralAsset.safeTransfer(msg.sender, reclaimAmount);

        emit CollateralReclaimed(reclaimAmount);
    }
}
